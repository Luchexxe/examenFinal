...
let resultado=[];
  pers.forEach((item)=>{
    let _empresas=[];
    let _duracion=0;
const arr=item.datos.laboral.experiencia.find((item2)=>item2.cargo===_cargo);

if(arr!=undefined)
{
  item.datos.laboral.experiencia.forEach((_e)=>{_empresas.push(_e.lugar)});
  item.datos.laboral.experiencia.forEach((_d)=>{_duracion=_duracion+_d.duracion;});
 resultado.push({nombre:item.nombre, cargo:_cargo,empresas:_empresas, duracion:_duracion });
}
...
eje9(personas,"Ejecutivo de ventas2");
eje9(personas,"Ejecutivo de ventas");