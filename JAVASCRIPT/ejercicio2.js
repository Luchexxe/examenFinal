...
function eje2(empresas, nomEmpresa, secEmpresa) {
  const emp = empresas.find((e) => e.nombre === nomEmpresa)
  if (!emp) {
    return "Empresa no encontrada";
  }
  const sec = emp.informacion.secciones.find((s) => s.nombre === secEmpresa);

  if (!sec) {
    return "Sección inválida para esta empresa";
  }

  const QEmpladosEmpresa = emp.informacion.empleados;
  const EmpAsignadosSeccion = sec.empleadosDestinados;
  return EmpAsignadosSeccion <= QEmpladosEmpresa;

}

const res = eje2(empresas, "Empresa3", "Turismo");
console.log(res);

const res2 = eje2(empresas, "EmpresaX", "Comercio");
console.log(res2);

const res3 = eje2(empresas, "Empresa3", "Comercio");
console.log(res3);
