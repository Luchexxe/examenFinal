...
function eje7(colegio)
{
  let resultado=[];
 
colegio.materiasDisponibles.forEach((item)=>{
  let arr;
 for(const profe of colegio.profesores)
 {
  for (const matHab of profe.materiasHabilitadas)
  {
      if(matHab===item)
      {
        arr="ok";
  ...
  if(arr==="ok")
  {
    resultado.push({materia:item, ok:true});
  }
  else
  {
    resultado.push({materia:item, ok:false});
  }
